...
def plot_image(x,fig_name='test.png', fig=None, ax=None, **kwargs):
    ...
    #ax.savefig("filename.png", bbox_inches = 'tight', pad_inches = 0)
    #ax.figure.savefig(fig_name ,bbox_inches = 'tight', pad_inches = 0)
    return im, ax


def plot_images(x_list, img_name='test.png', nrows=1, ncols=-1, fig=None, vrange='equal',
    ...
    ax_g = np.empty(ax.shape, dtype=object)
    for i, (x, ax_, v) in enumerate(zip(x_list, ax.flat, vrange_)):
        im_, ax_g = plot_image(x, img_name, ax=ax_, vmin=v[0], vmax=v[1], **kwargs)
        im.flat[i] = im_
        #if cbar == 'many':
